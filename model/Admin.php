...
JOIN sanpham ON chitietdonhang.id_sanpham_chitiet = sanpham.id_sanpham
 where donhang.id_taikhoan_donhang = $id and (donhang.trangthai_donhang='danggiao' or donhang.trangthai_donhang='choxacnhan') ;
";
        return $this->conn->query($sql)->fetchAll();
    }
...
";
    ...
    function giaothanhcong($id)
    {
        ...
        $result1 = $this->conn->query("UPDATE donhang SET trangthai_donhang = 'thanhcong' WHERE id_donhang = $id");
    
        $result2 = $this->conn->query("
            UPDATE sanpham
            JOIN chitietdonhang ON sanpham.id_sanpham = chitietdonhang.id_sanpham_chitiet
            SET sanpham.soluongtrongkho_sanpham = sanpham.soluongtrongkho_sanpham - chitietdonhang.soluong_chitiet,
                sanpham.sanphambanduoc_sanpham = sanpham.sanphambanduoc_sanpham + chitietdonhang.soluong_chitiet
            WHERE chitietdonhang.id_donhang_chitiet = $id
        ");
        if ($result1 && $result2) {
            return true; 
        }
    ...
    {
        $sql = "SELECT 
    DATE(donhang.thoigian_donhang) AS ngay, 
    SUM(chitietdonhang.soluong_chitiet * sanpham.gia_sanpham) AS doanhthu
FROM donhang
INNER JOIN chitietdonhang ON donhang.id_donhang = chitietdonhang.id_donhang_chitiet
INNER JOIN sanpham ON chitietdonhang.id_sanpham_chitiet = sanpham.id_sanpham
where donhang.trangthai_donhang='thanhcong'
GROUP BY DATE(donhang.thoigian_donhang)
ORDER BY ngay;
